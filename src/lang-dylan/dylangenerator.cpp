...
void DylanGenerator::writeFunction(QTextStream &s, const AbstractMetaFunction *meta_function,
    ...
    if (!dylan_function->isConstructor()) {
      ...
      libraryGenerator->addBinding(dylan_function->package(), dylan_function->dylanName());
    } else {
...
*/
    QString lid_file_name = dylan_class->package() + "/" + dylan_class->package().replace("-", "_") + ".lid";
